...
pub async fn execute(filter: Option<String>, output: Option<String>, html: bool) -> Result<()> {
    println!(
        "{}",
        "🚀 Running Icarus SDK Benchmarks".bright_cyan().bold()
    );
    ...
    if !cargo_toml.exists() {
        return Err(anyhow!("No Cargo.toml found. Please run this command from an Icarus project root or the SDK directory."));
    }
