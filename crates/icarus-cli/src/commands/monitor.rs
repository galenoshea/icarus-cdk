//! Monitoring dashboard for Icarus CDK
//!
...
impl MonitoringDashboard {
    ...
    pub async fn start(&self) -> Result<()> {
        println!("🔍 Starting Icarus CDK Monitoring Dashboard");
        println!(
    ...
    async fn collect_canister_health(&self, canister_id: Principal) -> Result<CanisterHealth> {
        let start = Instant::now();

        // Try to get real canister status from IC
        match self.get_canister_status(canister_id).await {
            Ok(status) => Ok(CanisterHealth {
                status: format!("{:?}", status.status),
                cycles: Some(status.cycles.0.to_u64().unwrap_or(0)),
                memory_usage: Some(status.memory_size.0.to_u64().unwrap_or(0)),
    ...
    /// Get real canister status from IC
    async fn get_canister_status(
        &self,
        canister_id: Principal,
    ) -> Result<ic_management_canister_types::CanisterStatusResult> {
        use ic_agent::Agent;
        use ic_management_canister_types::{CanisterStatusArgs, CanisterStatusResult};
        ...
        let request = CanisterStatusArgs { canister_id };

        let response_bytes = agent
            .update(&Principal::management_canister(), "canister_status")
            .with_arg(candid::encode_one(request)?)
            .call_and_wait()
            .await
            .map_err(|e| anyhow::anyhow!("Failed to get canister status: {}", e))?;

        let response: (CanisterStatusResult,) = candid::decode_one(&response_bytes)
            .map_err(|e| anyhow::anyhow!("Failed to decode canister status: {}", e))?;
    ...
    async fn collect_tool_usage(&self) -> Result<Vec<ToolUsage>> {
        // Try to get real tool usage from canister
        match self.get_tool_metrics().await {
            Ok(metrics) => Ok(metrics),
            Err(_) => {
                // Return empty usage if canister is unreachable
                Ok(vec![])
            }
        }
    }

    /// Get tool usage metrics from canister
    async fn get_tool_metrics(&self) -> Result<Vec<ToolUsage>> {
        use ic_agent::Agent;
    ...
    async fn display_table(&self, data: &MonitoringData) -> Result<()> {
        println!("🔍 Icarus CDK Monitoring Dashboard");
        println!(
            "⏰ Last updated: {} ms ago",
