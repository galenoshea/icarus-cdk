...
/// Display success message with appropriate styling (fixed for atomic printing)
pub fn display_success_animated(message: &str) {
    use std::io::{self, Write};

    // Separate emoji from colored text and flush immediately for atomic printing
    let formatted_message = format!("🎉 {}", message.green().bold());
...
pub fn display_error_styled(message: &str) {
    ...
    let formatted_message = format!("❌ {}", message.red().bold());
    println!("{}", formatted_message);
    io::stdout().flush().unwrap_or(());
}
...
pub fn display_warning_styled(message: &str) {
    ...
    let formatted_message = format!("⚠️ {}", message.yellow().bold());
    println!("{}", formatted_message);
    io::stdout().flush().unwrap_or(());
}
...
pub fn display_info_styled(message: &str) {
    ...
    let formatted_message = format!("ℹ️ {}", message.blue());
    println!("{}", formatted_message);
    io::stdout().flush().unwrap_or(());
}
